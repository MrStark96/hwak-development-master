@extends('layouts.app')

@section('header_scripts')
@endsection

@section('content')
<div class="heroSec aniView rent yellow" id="heroSec" style="background-image:url({{ asset('images/hawknest/4.jpg') }})">
    <div class="birdNestR"></div>
    <div class="NestL"></div>
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-5" style="overflow-y: hidden;">
                <div class="question">
                    What side of town?
                </div>

                <div class="container-fluid answers" style="height: 280px;">
                    <form class="w-100 h-100" action="/rent-step-4" method="post">
                        @csrf
                        <div id="map" style="width: 100%; height: 70%; margin-top: 10px; margin-bottom: 5px;">
                        </div>
                        <input id="location" name="location" type="hidden" value="" />
                        <button class="btn btn-warning" type="submit" id="nextButton"><span>Next</span><i class="fa fa-spinner fa-spin" style="display: none;"></i></button>
                    </form>
                </div>
            </div>
        </div>
    </div>
    <div class="toggleView">
        <input type="checkbox" id="toggleView">
        <label for="toggleView">
            <span>Animation</span>
            <span>Normal</span>
        </label>
    </div>
</div>
@endsection

@section('footer_scripts')
    <script type="text/javascript">
        let toggleViewValue = false;

        toggleViewValue = {!! json_encode($toggleViewRentValue) !!};
        let heroSec = document.getElementById('heroSec');
        let toggleView = document.getElementById('toggleView');

        if (toggleViewValue === "true") {
            heroSec.classList.remove("aniView");
            heroSec.classList.remove("normalView");
            heroSec.classList.add("normalView");
            toggleView.checked = true;
        } else {
            heroSec.classList.remove("aniView");
            heroSec.classList.remove("normalView");
            heroSec.classList.add("aniView");
            toggleView.checked = false;
        }

        let markersArray = [];
        let map;
        let polygon = null;
        let polygons = new Array();
        let latitude = {!! json_encode($latitude) !!};
        let longitude = {!! json_encode($longitude) !!};
        let uluru;
        let listner;
        let resetButton;
        let newBubbleButton;
        let stopButton;
        let startButton;
        let startFlag = false;

        function initMap() {
            uluru = { lat: parseFloat(latitude), lng: parseFloat(longitude) };

            map = new google.maps.Map(
                document.getElementById('map'),
                {
                    zoom: 13,
                    center: uluru,
                    mapTypeId: google.maps.MapTypeId.ROADMAP
                }
            );

            map.setOptions({ draggableCursor : "url(data:image/jpeg;base64,), auto" })

            const centerControlDiv = document.createElement('div');
            CustomDiv(centerControlDiv, map);
            centerControlDiv.index = 1;
            map.controls[google.maps.ControlPosition.TOP_CENTER].push(centerControlDiv);

            resetButton = document.getElementById('resetButton');
            newBubbleButton = document.getElementById('newBubbleButton');
            startButton = document.getElementById('mapStartButton');
            stopButton = document.getElementById('mapStopButton');
        }

        function newBubble() {
            resetButton = document.getElementById('resetButton');
            newBubbleButton = document.getElementById('newBubbleButton');
            startButton = document.getElementById('mapStartButton');
            stopButton = document.getElementById('mapStopButton');

            // #ffed4a
            newBubbleButton.style.backgroundColor = '#ffed4a';

            stopButton.setAttribute('disabled', 'false');
            stopButton.style.backgroundColor = 'grey';
            resetButton.setAttribute('disabled', 'false');
            resetButton.style.backgroundColor = 'grey';

            if (startFlag === false) {
                map.addListener('click', function(e) {
                    newBubbleButton.style.backgroundColor = 'grey';
                    startButton.style.backgroundColor = '#ffed4a';
                    addMarker(e.latLng);
                    drawPolygon();
                });

                startFlag = true;
            }

            markersArray.push(new Array());
        }

        function addMarker(latLng) {
            let marker = new google.maps.Marker({
                map: map,
                position: latLng,
                draggable: true
            });

            marker.addListener('position_changed', function() {
                drawPolygon();
            });

            markersArray[markersArray.length - 1].push(marker);
        }

        function clearMarkers() {
            markersArray.splice(0, markersArray.length);

            initMap();

            drawPolygon();

            startFlag = false;
        }

        function startMarkers() {
            startButton.setAttribute('disabled', 'true');
            startButton.style.pointerEvents = 'none';
            startButton.style.backgroundColor = '#a09d80';
            stopButton.setAttribute('disabled', 'false');
            stopButton.style.pointerEvents = 'auto';
            stopButton.style.backgroundColor = '#ffed4a';

            map.addListener('click', function(e) {
                addMarker(e.latLng);
                drawPolygon();
            });
        }

        function stopMarkers() {
            startButton.setAttribute('disabled', 'false');
            startButton.style.pointerEvents = 'auto';
            stopButton.setAttribute('disabled', 'true');
            stopButton.style.pointerEvents = 'none';
            stopButton.style.backgroundColor = '#ffed4a';
            startButton.style.backgroundColor = '#a09d80';

            startFlag = false;
            google.maps.event.clearListeners(map, 'click');
        }

        function drawPolygon() {
            let markersPositionArray = [];

            markersArray.forEach(function(markersPolygon) {
                let newPositionArray = new Array();

                markersPolygon.forEach(function(markersItem) {
                   newPositionArray.push(markersItem.getPosition());
                });

                markersPositionArray.push(newPositionArray);
            });

            polygons.forEach(function(polygonItem) {
                polygonItem.setMap(null);
            });

            polygons.splice(0, polygons.length);

            markersPositionArray.forEach(function(markersPosition) {
                polygon = new google.maps.Polygon({
                    paths: markersPosition,
                    strokeOpacity: 0.8,
                    strokeColor: 'red',
                    strokeWeight: 2,
                    fillColor: 'red',
                    fillOpacity: 0.35
                });

                polygons.push(polygon);

                // polygon.setMap(map);
            });

            polygons.forEach(function(polygonItem) {
                console.log(polygonItem);
                polygonItem.setMap(map)
            });

            $("#location").val('');

            let data = new Array();
            markersPositionArray.forEach(function(value) {
                data.push(value.toString());
            });

            $("#location").val(data);
        }

        function CustomDiv(controlDiv, map) {
            const controlUI = document.createElement("div");

            controlUI.classList.add('mapButtonsContainer');
            controlUI.style.textAlign = "center";
            controlUI.style.width = "400px";

            controlDiv.appendChild(controlUI);

            const resetButton = document.createElement("a");
            resetButton.id = 'resetButton';
            resetButton.setAttribute('disabled', 'true');
            resetButton.innerText = 'Reset';
            resetButton.style.padding = '5px 15px';
            resetButton.style.backgroundColor = 'grey';
            resetButton.addEventListener('click', function() { clearMarkers(); });

            const newBubbleButton = document.createElement("a");
            newBubbleButton.id = 'newBubbleButton';
            newBubbleButton.innerText = 'New Bubble';
            newBubbleButton.style.padding = '5px 15px';
            newBubbleButton.addEventListener('click', function() { newBubble(); });

            const mapStartButton = document.createElement("a");
            mapStartButton.id = 'mapStartButton';
            mapStartButton.setAttribute('disabled', 'true');
            mapStartButton.innerText = 'Start Mark';
            mapStartButton.style.padding = '5px 15px';
            mapStartButton.style.backgroundColor = 'grey';
            mapStartButton.addEventListener('click', function() { startMarkers(); });

            const mapStopButton = document.createElement("a");
            mapStopButton.id = 'mapStopButton';
            mapStopButton.setAttribute('disabled', 'true');
            mapStopButton.innerText = 'Stop Mark';
            mapStopButton.style.padding = '5px 15px';
            mapStopButton.style.backgroundColor = 'grey';
            mapStopButton.addEventListener('click', function() { stopMarkers(); });

            controlUI.appendChild(resetButton);
            controlUI.appendChild(newBubbleButton);
            controlUI.appendChild(mapStartButton);
            controlUI.appendChild(mapStopButton);
        }
    </script>
@endsection
